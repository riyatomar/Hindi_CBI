...
from scripts.ner_handler import integrate_ner_annotations
from scripts.parser_operations import (
    handle_mod_and_head,
    handle_pof_rvks_rbk,
    handle_measurement_units,
    handle_calendaric_units,
    handle_spans,
    handle_spatial_relations,
    handle_conjunctions
)
...
def modify_json_data(json_data, ner_data):
    span_count = nc_count = cp_count = 1
    meas_count = calendaric_count = 1
    disjunct_count = conj_count = spatial_count = ne_count = 1
    new_entries = []
